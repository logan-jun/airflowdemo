        ...
        Steps=[
            {
                ...
                'HadoopJarStep': {
                    ...
                    'Args': [
                        'spark-submit', '--deploy-mode', 'client', '--class', 'com.example.beam.App', 's3://bsjun-test1/beamapp/beam-app-1.0.0-shaded.jar', '--runner=SparkRunner', '--inputPath=s3://bsjun-test1/before/output.csv', '--outputPath=s3://bsjun-test1/results/output/result'
                    ]
                }
            },
        ]
...
def create_cluster(region_name, cluster_name='Airflow-' + str(datetime.now()), release_label='emr-5.9.0',master_instance_type='m3.xlarge', num_core_nodes=1, core_node_instance_type='m3.xlarge'):
    ...
    cluster_response = emr.run_job_flow(
        ...
        LogUri='s3n://bsjun-test1/emrLogs',
        Instances={
            'InstanceGroups': [
                {
                    'Name': "Master nodes",
